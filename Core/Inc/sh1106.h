...
// SH1106 RS/A0 (Data/Command select) pin (PB10)
...
// SH1106 RST (Reset) pin (PB11)
#define SH1106_RST_PORT GPIOB
#define SH1106_RST_PIN GPIO_PIN_11
#define SH1106_RST_H()                                                         \
  HAL_GPIO_WritePin(SH1106_RST_PORT, SH1106_RST_PIN, GPIO_PIN_SET)
#define SH1106_RST_L()                                                         \
  HAL_GPIO_WritePin(SH1106_RST_PORT, SH1106_RST_PIN, GPIO_PIN_RESET)
...
#define SH1106_CMD_SETMUX                                                      \
  (uint8_t)0xA8 // Set multiplex ratio (N, number of lines active on display)
#define SH1106_CMD_SETOFFS (uint8_t)0xD3   // Set display offset
#define SH1106_CMD_STARTLINE (uint8_t)0x40 // Set display start line
#define SH1106_CMD_SEG_NORM                                                    \
  (uint8_t)0xA0 // Column 0 is mapped to SEG0 (X coordinate normal)
#define SH1106_CMD_SEG_INV                                                     \
  (uint8_t)0xA1 // Column 127 is mapped to SEG0 (X coordinate inverted)
#define SH1106_CMD_COM_NORM                                                    \
  (uint8_t)0xC0 // Scan from COM0 to COM[N-1] (N - mux ratio, Y coordinate
                // normal)
#define SH1106_CMD_COM_INV                                                     \
  (uint8_t)0xC8 // Scan from COM[N-1] to COM0 (N - mux ratio, Y coordinate
                // inverted)
...
#define SH1106_CMD_EDON                                                        \
  (uint8_t)0xA5 // Entire display ON enabled (all pixels on, RAM content
                // ignored)
#define SH1106_CMD_EDOFF                                                       \
  (uint8_t)0xA4 // Entire display ON disabled (output follows RAM content)
#define SH1106_CMD_INV_OFF                                                     \
  (uint8_t)0xA6 // Entire display inversion OFF (normal display)
#define SH1106_CMD_INV_ON                                                      \
  (uint8_t)0xA7 // Entire display inversion ON (all pixels inverted)
#define SH1106_CMD_CLOCKDIV                                                    \
  (uint8_t)0xD5 // Set display clock divide ratio/oscillator frequency
...
#define SH1106_CMD_SCRL_HR                                                     \
  (uint8_t)0x26 // Setup continuous horizontal scroll right
#define SH1106_CMD_SCRL_HL                                                     \
  (uint8_t)0x27 // Setup continuous horizontal scroll left
#define SH1106_CMD_SCRL_VHR                                                    \
  (uint8_t)0x29 // Setup continuous vertical and horizontal scroll right
#define SH1106_CMD_SCRL_VHL                                                    \
  (uint8_t)0x2A // Setup continuous vertical and horizontal scroll left
