...
// Pixel drawing mode
// Whereas in most drawing operations pixels are set, use global variable to
// select drawing mode instead of passing set/reset/invert mode in each call of
// drawing functions
...
static const uint8_t LUT_FB[] = {0x00, 0x80, 0xC0, 0xE0,
                                 0xF0, 0xF8, 0xFC, 0xFE};

// Vertical line drawing look up table for last byte
static const uint8_t LUT_LB[] = {0x00, 0x01, 0x03, 0x07,
                                 0x0F, 0x1F, 0x3F, 0x7F};
...
void SH1106_Init(void) {
  ...
  // bits[7:4] set the oscillator frequency (Fosc), frequency increases with the
  // value of these bits 0xF0 value gives maximum frequency (maximum Fosc
  // without divider) 0x0F value gives minimum frequency (minimum Fosc divided
  // by 16) The higher display frequency decreases image flickering but
  // increases current consumption and vice versa
...
// note: this setting specifies an orientation of display, not orientation of
// image
...
void SH1106_Flush(void) {
  ...
  uint8_t ram_pointer[] = {SH1106_CMD_COL_LOW | 0x02, SH1106_CMD_COL_HIGH, 0};
  ...
  for (register uint32_t page = 0; page < bits_h; ++page) {
    ...
    HAL_SPI_Transmit(&SH1106_SPI_PORT, (uint8_t *)ram_pointer,
                     sizeof(ram_pointer), 1000);
...
//   interval - time interval between scroll steps (one of LCD_SCROLL_IFXXX
//   values)
void SH1106_ScrollHSetup(uint8_t dir, uint8_t start, uint8_t end,
                         uint8_t interval) {
  SH1106_CS_L();
  SH1106_cmd((dir == LCD_SCROLL_RIGHT) ? SH1106_CMD_SCRL_HR
                                       : SH1106_CMD_SCRL_HL);
  SH1106_cmd(0x00);     // dummy byte
  SH1106_cmd(start);    // Start page address
  SH1106_cmd(interval); // Time interval between each scroll stop in terms of
                        // frame frequency
...
//   end - end page address [0..7], must be great or equal to start value
//   interval - time interval between scroll steps (one of LCD_SCROLL_IFXXX
//   values) voffs - vertical scrolling offset, this value specifies how many
//   lines will
...
void SH1106_ScrollDSetup(uint8_t dir, uint8_t start, uint8_t end,
  ...
  SH1106_CS_L();
  SH1106_cmd((dir == LCD_SCROLL_RIGHT) ? SH1106_CMD_SCRL_VHR
                                       : SH1106_CMD_SCRL_VHL);
  SH1106_cmd(0x00);     // dummy byte
  SH1106_cmd(start);    // Start page address
  SH1106_cmd(interval); // Time interval between each scroll stop in terms of
...
#if (SH1106_OPT_PIXEL)
__attribute__((always_inline)) void LCD_Pixel(uint8_t X, uint8_t Y,
                                              uint8_t Mode) {
#else
void LCD_Pixel(uint8_t X, uint8_t Y, uint8_t Mode) {
  ...
  // Offset of pixel in the vRAM array must be computed by formula ((Y >> 3) *
  // SCR_W) + X Since screen is 128 pixel width the formula can be simplified to
  // ((Y >> 3) << 7) + X For 90 degree rotation X and Y must be swapped
...
void LCD_FillRect(uint8_t X1, uint8_t Y1, uint8_t X2, uint8_t Y2) {
  ...
  // Fill rectangle by vertical lines is most optimal, therefore calculate
  // coordinates with regard of screen rotation
  if (scr_orientation == LCD_ORIENT_CW || scr_orientation == LCD_ORIENT_CCW) {
    if (X1 > X2) {
      T = X2;
      L = X1 - X2;
...
void LCD_Circle(int16_t Xc, int16_t Yc, uint8_t R) {
  ...
  while (x < y) {
    ...
    if (Xc + y < sw) {
      if (Yc + x < sh)
        LCD_Pixel(Xc + y, Yc + x, LCD_PixelMode);
      if (Yc - x > -1)
        LCD_Pixel(Xc + y, Yc - x, LCD_PixelMode);
    }
    if (Xc - y > -1) {
  ...
  if (Xc + R < sw)
    LCD_Pixel(Xc + R, Yc, LCD_PixelMode);
  if (Xc - R > -1)
    LCD_Pixel(Xc - R, Yc, LCD_PixelMode);
  if (Yc + R < sh)
    LCD_Pixel(Xc, Yc + R, LCD_PixelMode);
  if (Yc - R > -1)
    LCD_Pixel(Xc, Yc - R, LCD_PixelMode);
}
...
uint8_t LCD_PutChar(uint8_t X, uint8_t Y, uint8_t Char,
  ...
  // If the specified character code is out of bounds should substitute the code
  // of the "unknown" character
  if (Char < Font->font_MinChar || Char > Font->font_MaxChar)
    Char = Font->font_UnknownChar;
...
//   Font - pointer to font
// return: string width in pixels
uint16_t LCD_PutStrLF(uint8_t X, uint8_t Y, const char *str,
                      const Font_TypeDef *Font) {
  uint8_t strLen = 0;
...
uint8_t LCD_PutInt(uint8_t X, uint8_t Y, int32_t num,
                   const Font_TypeDef *Font) {
...
//   Font - pointer to font
// return: number width in pixels
uint8_t LCD_PutIntU(uint8_t X, uint8_t Y, uint32_t num,
                    const Font_TypeDef *Font) {
...
//   Font - pointer to font
// return: number width in pixels
uint8_t LCD_PutIntF(uint8_t X, uint8_t Y, int32_t num, uint8_t decimals,
                    const Font_TypeDef *Font) {
  uint8_t str[11]; // 10 chars max for INT32_MIN..INT32_MAX (without sign)
  uint8_t *pStr = str;
  uint8_t pX = X;
  ...
  *pStr++ = '\0';
  ...
  while (*--pStr) {
    ...
    if (decimals && (--strLen == decimals)) {
      // Draw decimal point
      LCD_Rect(pX, Y + Font->font_Height - 2, pX + 1,
...
//   Font - pointer to font
// return: number width in pixels
uint8_t LCD_PutIntLZ(uint8_t X, uint8_t Y, int32_t num, uint8_t digits,
                     const Font_TypeDef *Font) {
  uint8_t str[11]; // 10 chars max for INT32_MIN..INT32_MAX (without sign)
  uint8_t *pStr = str;
  uint8_t pX = X;
...
//   Font - pointer to font
// return: number width in pixels
uint8_t LCD_PutHex(uint8_t X, uint8_t Y, uint32_t num,
                   const Font_TypeDef *Font) {
  uint8_t str[11]; // 10 chars max for UINT32_MAX
  uint8_t *pStr = str;
  uint8_t pX = X;
  ...
  while (pY < Y + H) {
    ...
    while (pX < X + W) {
      ...
      if (tmpCh) {
        while (bL < 8) {
          if (tmpCh & 0x01)
            LCD_Pixel(pX, pY + bL, LCD_PixelMode);
          tmpCh >>= 1;
          if (tmpCh) {
            bL++;
          } else {
