...
/// Static LED buffer
volatile u8_t RGB_BUF[NUM_BYTES] = {
    0,
};

/// Timer PWM value buffer
volatile dma_siz PWM_BUF[PWM_BUF_LEN] = {
    0,
};
/// PWM buffer iterator
...
void ARGB_Init(void) {
  ...
  TIM_CCxChannelCmd(TIM_HANDLE.Instance, TIM_CH,
 ...
 */
void ARGB_SetBrightness(u8_t br) { ARGB_BR = br; }
 ...
 */
ARGB_STATE ARGB_Ready(void) { return ARGB_LOC_ST; }
...
ARGB_STATE ARGB_Show(void) {
  ...
  if (BUF_COUNTER != 0 || DMA_HANDLE.State != HAL_DMA_STATE_READY) {
    ...
    while (DMA_Send_Stat != HAL_OK) {
      if (TIM_CHANNEL_STATE_GET(&TIM_HANDLE, TIM_CH) ==
          HAL_TIM_CHANNEL_STATE_BUSY) {
        DMA_Send_Stat = HAL_BUSY;
        continue;
      } else if (TIM_CHANNEL_STATE_GET(&TIM_HANDLE, TIM_CH) ==
...
#endif
      TIM_HANDLE.hdma[ARGB_TIM_DMA_ID]->XferCpltCallback =
          ARGB_TIM_DMADelayPulseCplt;
      TIM_HANDLE.hdma[ARGB_TIM_DMA_ID]->XferHalfCpltCallback =
...
static void ARGB_TIM_DMADelayPulseCplt(DMA_HandleTypeDef *hdma) {
  ...
  // if wrong handlers
  if (hdma != &DMA_HANDLE || htim != &TIM_HANDLE)
    return;
  if (BUF_COUNTER == 0)
  ...
  if (BUF_COUNTER < NUM_PIXELS) {
    ...
    for (volatile u8_t i = 0; i < 8; i++) {
      ...
      PWM_BUF[i + 40] =
          (((RGB_BUF[4 * BUF_COUNTER + 1] << i) & 0x80) > 0) ? PWM_HI : PWM_LO;
      PWM_BUF[i + 48] =
    ...
    memset((dma_siz *)&PWM_BUF[PWM_BUF_LEN / 2], 0,
...
// Check channel
#if !(TIM_CH == TIM_CHANNEL_1 || TIM_CH == TIM_CHANNEL_2 ||                    \
      TIM_CH == TIM_CHANNEL_3 || TIM_CH == TIM_CHANNEL_4)
